...
def compare_sort(sizes=[100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000]):
      ...
      for size in sizes:
          ...
          random.shuffle(mylist)  # Ensure the list is shuffled for each test
          fixed_pivot_time = time_search(qsort_fixed_pivot, mylist.copy())
          random_pivot_time = time_search(qsort_random_pivot, mylist.copy())
          timsort_time = time_search(sorted, mylist.copy())  # Measure Python's sorted() function
          result.append([size, fixed_pivot_time, random_pivot_time, timsort_time])
      return result
